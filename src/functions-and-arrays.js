...
function sum(numbers) {
	...
	for (let i = 0; i < numbers.length; i++) {
		...
		if (typeof numbers[i] === 'number') {
			...
			// PEOPLE, ARE YOU OK WITH THE TESTS??? SHOULD I MAKE THE TASKS AS THE TESTS ARE WRITTEN?
		}
	}
...
function averageNumbers(numbersAvg) {
	...
	averageOfNumbers = sum / numbersAvg.length;
...
function avg(arrayMix) {
	...
	for (let i = 0; i < arrayMix.length; i++) {
		if (typeof arrayMix[i] === 'number') {
			summNumbers += +arrayMix[i];
		} else if (typeof arrayMix[i] === 'string') {
			summNumbers += arrayMix[i].length;
		} else if (arrayMix[i] === true) {
			summNumbers += 1;
		}
	}
	//   return summNumbers / arrayMix.length;
